...
function computeInterquartileRangeAbs(values) {
  const upper = computePercentileAbs(values, 0.75)
  const lower = computePercentileAbs(values, 0.25)
...
function buildMeasurementStats(delta) {
  return {
    ...
    spl_interquartile_range_db: computeInterquartileRangeAbs(delta.spl_delta_db),
