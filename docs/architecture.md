...
_Progress snapshot:_ M1 and M2 remain complete while M3 now closes at 100 % after adding piston directivity traces with directivity index telemetry—now surfaced in the Studio spectra overlay, augmented with -6 dB beamwidth summaries, and exportable via a standalone hybrid directivity CLI—on top of the earlier SPL correlation/MAE/R²/P95/median absolute deviation metrics and the new SPL interquartile-range reporting.  This iteration additionally introduces Prometheus-compatible gateway instrumentation on `/metrics` so request, solver, run-store, and measurement comparison latency telemetry are observable by default, nudging the overall roadmap about 77 % toward v1.0.
...
- **Observability**: Structured logging roadmap (loguru) paired with the shipped FastAPI `/metrics` endpoint exposing Prometheus counters/histograms for HTTP traffic, solver duration, measurement comparison latency, and run-store gauges via the optional `prometheus-client`, with OpenTelemetry export still toggled by env var in upcoming work.
