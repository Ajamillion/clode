...
- **Milestone M3 – High-Fidelity Solvers:** ≈94 % complete. The hybrid solver now exports multi-plane interior pressure slices with per-plane statistics, records hotspot coordinates, reports minimum port compression ratios, introduces a configurable snapshot stride so APIs can thin out interior rasters without losing the aggregated summary metrics, models suspension creep with per-run telemetry plus an API toggle, simulates a lightweight thermal network estimating coil/pole/basket heating, and now reruns measurement comparisons automatically with derived calibration overrides so baseline and calibrated fits are delivered together. Band-limited analysis remains available across the CLI, gateway, and Studio UI so teams can focus on the frequency spans that matter most while the FEM/BEM adaptor matures behind the scenes.
- **Overall programme:** ≈74 % toward the v1.0 roadmap ((1.0 + 1.0 + 0.94 + 0) ÷ 4 milestones).
- Latest iteration layers the thermal telemetry and compression metadata on top of the snapshot stride control, suspension creep model, and hotspot mapping work, keeping hybrid telemetry tunable while giving Studio and CLI users precise control over SPL/impedance fit windows and temperature headroom.
