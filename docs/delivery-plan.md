...
- **Milestone M3 – High-Fidelity Solvers:** 100 % complete. The reduced-order hybrid solver now layers vortex-shedding attenuation, jet-noise SPL estimation referenced to microphone distance, piston directivity traces with directivity index telemetry and -6 dB beamwidth summaries, suspension creep toggles, thermal telemetry, and interior hotspot mapping, while the supporting toolchain covers multi-metric measurement overlays, exportable comparison datasets, Docker Compose orchestration, the Playwright Studio regression suite, and SPL interquartile-range reporting for measurement comparisons.
- **Overall programme:** ≈77 % toward the v1.0 roadmap ((1.0 + 1.0 + 1.0 + 0.08) ÷ 4 milestones).
...
- The FastAPI gateway now exposes a Prometheus `/metrics` endpoint that records HTTP latency, solver runtime, measurement comparison latency, and run-store counts whenever `prometheus-client` is installed, giving the observability workstream a concrete starting point ahead of the wider M4 hardening goals.
...
- Layer observability (structured logs + extended Prometheus metrics) into the gateway and solver workers, wiring Grafana dashboards for runtime health.
