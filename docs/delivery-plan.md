...
- **Milestone M3 – High-Fidelity Solvers:** ≈82 % complete. The hybrid solver now exports multi-plane interior pressure slices with per-plane statistics, records hotspot coordinates, reports minimum port compression ratios, and introduces a configurable snapshot stride so APIs can thin out interior rasters without losing the aggregated summary metrics. Measurement comparisons now support band-limited analysis across the CLI, gateway, and Studio UI so teams can focus on the frequency spans that matter most while the FEM/BEM adaptor matures behind the scenes.
- **Overall programme:** ≈70.5 % toward the v1.0 roadmap ((1.0 + 1.0 + 0.82 + 0) ÷ 4 milestones).
- Latest iteration layers the snapshot stride control on top of hotspot coordinate mapping and adds frequency-band gating for measurement comparisons, keeping hybrid telemetry tunable while giving Studio and CLI users precise control over SPL/impedance fit windows.
