...
class HybridFieldSnapshot:
    ...
    port_vortex_loss_db: float | None
    port_noise_spl_db: float | None
    plane_label: str
    ...
    def to_dict(self, *, include_pressure: bool = True) -> dict[str, Any]:
        ...
        data: dict[str, Any] = {
            ...
            "port_vortex_loss_db": self.port_vortex_loss_db,
            "port_noise_spl_db": self.port_noise_spl_db,
...
class HybridSolverSummary:
    ...
    max_port_vortex_loss_db: float | None = None
    max_port_noise_spl_db: float | None = None
    port_noise_reference_distance_m: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "max_port_vortex_loss_db": self.max_port_vortex_loss_db,
            "max_port_noise_spl_db": self.max_port_noise_spl_db,
            "port_noise_reference_distance_m": self.port_noise_reference_distance_m,
...
class HybridBoxSolver:
    ...
    def __init__(
        ...
        *,
        ...
        self._vortex_onset_ms = 5.0
        self._port_noise_reference_m = 1.0
        self._jet_noise_scale = 1e3
    ...
    def frequency_response(
        ...
        *,
        ...
        port_vortex_losses: list[float] = []
        port_noise_levels: list[float] = []
        vortex_loss_samples: list[float] = []
        ...
        for freq in frequencies_hz:
                ...
                if snapshot_index % snapshot_stride == 0:
                    snapshots.append(
                        HybridFieldSnapshot(
                            ...
                            port_vortex_loss_db=vortex_loss,
                            port_noise_spl_db=jet_noise,
            ...
            if vortex_loss is not None:
                port_vortex_losses.append(vortex_loss)
                vortex_loss_samples.append(vortex_loss)
            else:
                port_vortex_losses.append(0.0)
            if jet_noise is not None:
                port_noise_levels.append(jet_noise)
                noise_level_samples.append(jet_noise)
            else:
                port_noise_levels.append(0.0)
        ...
        max_vortex_loss = max(vortex_loss_samples) if vortex_loss_samples else None
        max_noise_level = max(noise_level_samples) if noise_level_samples else None
        summary = HybridSolverSummary(
            ...
            max_port_vortex_loss_db=max_vortex_loss if self._mode == "vented" else None,
            max_port_noise_spl_db=max_noise_level if self._mode == "vented" else None,
            port_noise_reference_distance_m=(
                self._port_noise_reference_m if self._mode == "vented" else None
            ),
    ...
    def _vented_state(
        ...
        vortex_loss_db: float | None = None
        jet_noise_db: float | None = None
        if compression is not None and compression < 1.0:
            port_volume_velocity *= compression
        if port_velocity is not None and port_velocity > 0.0:
            adjusted_velocity, raw_vortex_loss = self._apply_vortex_shedding(port_velocity)
            if adjusted_velocity <= 0.0:
                adjusted_velocity = port_velocity
                raw_vortex_loss = 0.0
            ratio = adjusted_velocity / port_velocity if port_velocity > 0 else 1.0
            port_velocity = adjusted_velocity
            if port_volume_velocity is not None:
                port_volume_velocity *= ratio
            if raw_vortex_loss > 1e-3:
                vortex_loss_db = raw_vortex_loss
            freq_hz = omega / (2 * pi)
            jet_noise_estimate = self._estimate_port_noise(port_velocity, freq_hz, port_area)
            if jet_noise_estimate > 1.0:
                jet_noise_db = jet_noise_estimate
    ...
    def _apply_vortex_shedding(self, velocity: float) -> tuple[float, float]:
        if velocity <= 0.0:
            return 0.0, 0.0
        if velocity <= self._vortex_onset_ms:
            return velocity, 0.0
        ratio = (velocity - self._vortex_onset_ms) / self._vortex_onset_ms
        attenuation = 1.0 / (1.0 + ratio * ratio)
        attenuation = max(min(attenuation, 1.0), 0.05)
        adjusted = velocity * attenuation
        loss_db = -20.0 * log10(max(attenuation, 1e-6))
        return adjusted, loss_db

    def _estimate_port_noise(self, velocity: float, frequency_hz: float, port_area: float) -> float:
        if velocity <= 0.0:
            return 0.0
        diameter = sqrt(max(4.0 * port_area / pi, 1e-12))
        mach = velocity / SPEED_OF_SOUND
        if mach <= 0.01:
            return 0.0
        reference_distance = max(self._port_noise_reference_m, 0.05)
        base_power = (
            AIR_DENSITY
            * (velocity**8)
            * (diameter**2)
            / (SPEED_OF_SOUND**5 * reference_distance**2)
        )
        weighted_power = max(base_power * self._jet_noise_scale, 1e-18)
        spectral_weight = (max(frequency_hz, 1.0) / 100.0) ** 0.3
        power = weighted_power * spectral_weight
        spl = 10.0 * log10(power) + 120.0
        return max(spl, 0.0)
