...
class HybridBoxSolver:
        ...
        *,
        ...
        for freq in frequencies_hz:
            ...
            if self._mode == "sealed":
                ...
                (
                    ...
                    vortex_loss,
                    jet_noise,
                ) = self._vented_state(omega, port_noise_reference_m)
        ...
        summary = HybridSolverSummary(
            ...
            port_noise_reference_distance_m=(
                port_noise_reference_m if self._mode == "vented" else None
            ),
    ...
    def _vented_state(
        ...
        if port_velocity is not None and port_velocity > 0.0:
            ...
            freq_hz = omega / (2 * pi)
            jet_noise_estimate = self._estimate_port_noise(
                port_velocity,
                freq_hz,
                port_area,
                port_noise_reference_m,
            )
    ...
    def _estimate_port_noise(
        ...
        if mach <= 0.01:
            return 0.0
        reference_distance = max(reference_distance_m, 0.05)
        base_power = (
            AIR_DENSITY
            * (velocity**8)
            * (diameter**2)
