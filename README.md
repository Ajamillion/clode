...
- ✅ Measurement stats now report the SPL interquartile range so reviewers can quickly gauge mid-spread behaviour between the 25th and 75th percentile deltas
...
- ✅ Gateway exposes a `/metrics` endpoint with Prometheus-compatible request, solver, run-store, and measurement comparison latency instrumentation (enabled when `prometheus-client` is installed)
...
Current roadmap snapshot: **M1 100 %**, **M2 100 %**, **M3 100 %**, overall ≈77 % toward the v1.0 target.
...
2. (Optional) Install Python dev dependencies for lint/type/test workflows
   ...
   pip install -e .[dev]
   ```
3. (Optional) Enable the Prometheus metrics endpoint for the FastAPI gateway
   ```bash
   pip install -e .[metrics]
   ```
4. In one terminal start the mock backend & WebSocket feed (includes the optimisation run API)
   ```bash
   pnpm --filter @bagger/dev-mocks dev
   ```
5. In another terminal launch the Studio web client
   ```bash
   pnpm --filter @bagger/web-ui dev
   ```
6. Open http://localhost:5173 to view the demo enclosure renderer streaming synthetic optimization telemetry.
...
`/health`, `/opt`, `/simulate`, `/measurements`, and `/metrics` routes) and the Studio UI at
...
`pnpm --filter @bagger/web-ui test:e2e:report`.
