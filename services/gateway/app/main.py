...
try:  # pragma: no cover - optional dependency
    ...
    def generate_latest(*_: Any, **__: Any) -> bytes:
        return b""
...
PROMETHEUS_ENABLED = Counter is not None and Gauge is not None and Histogram is not None

if PROMETHEUS_ENABLED:  # pragma: no cover - exercised in integration environments
    REQUEST_COUNTER = Counter(
        "bagger_gateway_requests_total",
        "HTTP requests processed by the gateway",
        ("endpoint", "method", "status"),
    )
    REQUEST_LATENCY = Histogram(
        "bagger_gateway_request_latency_seconds",
        "Latency of gateway HTTP requests",
        ("endpoint", "method"),
        buckets=(0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0),
    )
    SOLVER_LATENCY = Histogram(
        "bagger_gateway_solver_duration_seconds",
        "Duration of solver operations executed by the gateway",
        ("solver",),
        buckets=(0.001, 0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0),
    )
    RUN_STATUS_GAUGE = Gauge(
        "bagger_gateway_runs",
        "Persisted optimisation runs by status",
        ("status",),
    )
    MEASUREMENT_COMPARISON_LATENCY = Histogram(
        "bagger_gateway_measurement_comparison_latency_seconds",
        "Duration of measurement comparison payload assembly",
        ("alignment", "calibrated"),
        buckets=(0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0),
    )
else:  # pragma: no cover - default fallback when prometheus_client is unavailable
    REQUEST_COUNTER = None
    REQUEST_LATENCY = None
    SOLVER_LATENCY = None
    RUN_STATUS_GAUGE = None
    MEASUREMENT_COMPARISON_LATENCY = None
...
def _record_http_metrics(endpoint: str, method: str, status: str, duration: float) -> None:
...
@contextmanager
def _observe_solver_duration(solver: str) -> Any:
    start = perf_counter()
    try:
        yield
    finally:
        if SOLVER_LATENCY is not None:
            SOLVER_LATENCY.labels(solver=solver).observe(perf_counter() - start)


@contextmanager
def _observe_measurement_comparison(alignment: str, apply_overrides: bool) -> Any:
    start = perf_counter()
    try:
        yield
    finally:
        if MEASUREMENT_COMPARISON_LATENCY is not None:
            MEASUREMENT_COMPARISON_LATENCY.labels(
                alignment=alignment,
                calibrated="true" if apply_overrides else "false",
            ).observe(perf_counter() - start)


def _update_run_metrics(store: RunStore | None) -> None:
    if RUN_STATUS_GAUGE is None or store is None:
        return
    counts = store.status_counts()
    for status in VALID_STATUSES:
        RUN_STATUS_GAUGE.labels(status=status).set(float(counts.get(status, 0)))
...
def _sealed_measurement_payload(
    ...
    with _observe_solver_duration("sealed_frequency_response"):
...
def _vented_measurement_payload(
    ...
    with _observe_solver_duration("vented_frequency_response"):
        response = solver.frequency_response(measurement.frequency_hz, mic_distance_m)
    summary = solver.alignment_summary(response)
    predicted = measurement_from_response(response)
    delta, stats, diagnosis = compare_measurement_to_prediction(
...
def _measurement_comparison_payload(
    *,
    ...
    with _observe_measurement_comparison(alignment, apply_overrides):
        if alignment == "vented":
...
class SealedMeasurementRequest(BaseModel):
    ...
    smoothing_fraction: float | None = Field(None, gt=0)
...
def _run_optimisation_task(run_id: str, params: dict[str, Any]) -> None:
    ...
    try:
        ...
        with _observe_solver_duration("optimisation_result"):
...
if FastAPI is not None:  # pragma: no branch
    ...
    if PROMETHEUS_ENABLED:
        ...
        async def _prometheus_http_middleware(request: Request, call_next):
            ...
            try:
                ...
                _record_http_metrics(request.url.path, request.method, "500", duration)
                raise
            duration = perf_counter() - start
            status = str(getattr(response, "status_code", 500))
            _record_http_metrics(request.url.path, request.method, status, duration)
            return response
    ...
    async def compare_sealed_measurement(payload: SealedMeasurementRequest) -> dict[str, Any]:
        ...
        return _measurement_comparison_payload(
            ...
            smoothing_fraction=payload.smoothing_fraction,
        )
    ...
    async def simulate_hybrid(payload: HybridRequest) -> dict[str, Any]:
        ...
        with _observe_solver_duration("hybrid_frequency_response"):
    ...
    async def sealed_tolerances(payload: SealedToleranceRequest) -> dict[str, Any]:
        ...
        with _observe_solver_duration("sealed_tolerance_analysis"):
    ...
    async def vented_tolerances(payload: VentedToleranceRequest) -> dict[str, Any]:
        spec = _tolerance_spec_from_payload(payload.tolerances)
        with _observe_solver_duration("vented_tolerance_analysis"):
            report = run_tolerance_analysis(
                "vented",
                payload.driver.to_driver(),
                payload.box.to_box(),
